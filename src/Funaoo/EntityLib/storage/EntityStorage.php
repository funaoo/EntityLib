...
final class EntityStorage {
    ...
    public function delete(int $entityId): bool {
        $data = $this->provider->load();
        if (!isset($data[(string)$entityId])) return false;
    ...
    public function loadAll(): int {
        ...
        foreach ($this->provider->load() as $row) {
            ...
            try {
                if ($this->spawnFromData(EntityData::fromArray($row))) $loaded++;
            } catch (\Throwable $e) {
    ...
    public function clearAll(): bool { return $this->provider->delete(); }
    public function backup(): bool   { return $this->provider->backup(); }
    public function count(): int     { return count($this->provider->load()); }

    private function spawnFromData(EntityData $ed): bool {
        $wm = Server::getInstance()->getWorldManager();

        if (!$wm->isWorldLoaded($ed->world) && !$wm->loadWorld($ed->world)) {
        ...
        $builder = EntityLib::create(new Vector3($ed->x, $ed->y, $ed->z), $world)
            ->setType($ed->type)
            ->setName($ed->name)
            ->setSkin(new Skin($ed->skinId !== '' ? $ed->skinId : 'Standard_Custom', $skinBytes))
            ->setScale($ed->scale)
        ...
        if (isset($ed->metadata['__interactionCommand']) && is_string($ed->metadata['__interactionCommand'])) {
            ...
            $id  = $entity->getId();
            EntityLib::getInteractionHandler()->register($id, static function(\pocketmine\player\Player $player) use ($cmd): void {

...
final class EffectManager {
    ...
    public function removeEffect(int $entityId, int $index): void {
        ...
        if ($this->effects[$entityId] === []) unset($this->effects[$entityId]);
        if ($this->effects === []) $this->stopTask();
    }

    public function hasEffects(int $entityId): bool { return isset($this->effects[$entityId]) && $this->effects[$entityId] !== []; }
    public function getEffects(int $entityId): array { return $this->effects[$entityId] ?? []; }
    public function isRunning(): bool               { return $this->task !== null; }

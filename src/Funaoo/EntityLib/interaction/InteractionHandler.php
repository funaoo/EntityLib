...
final class InteractionHandler {
    ...
    public function hasCallback(int $entityId): bool { return isset($this->callbacks[$entityId]); }

    public function handleInteraction(int $entityId, Player $player, BaseEntity $entity): void {
        if (!isset($this->callbacks[$entityId]) || $this->cooldowns->isOnCooldown($entityId, $player)) return;
    ...
    public function setDefaultCooldown(float $seconds): void { $this->defaultCooldown = max(0.0, $seconds); }
    public function getDefaultCooldown(): float              { return $this->defaultCooldown; }
    public function getCooldownManager(): CooldownManager    { return $this->cooldowns; }
    public function getRegisteredEntityIds(): array          { return array_keys($this->callbacks); }

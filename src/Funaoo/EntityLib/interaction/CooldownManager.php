...
final class CooldownManager {
    ...
    public function isOnCooldown(int $entityId, Player $player): bool {
        ...
        if (!isset($this->cooldowns[$entityId][$key])) return false;
    ...
    public function getRemaining(int $entityId, Player $player): float {
        $key = strtolower($player->getName());
        return isset($this->cooldowns[$entityId][$key])
            ? max(0.0, $this->cooldowns[$entityId][$key] - microtime(true))
            : 0.0;
    }

    public function clearEntity(int $entityId): void { unset($this->cooldowns[$entityId]); }
    public function clearAll(): void                 { $this->cooldowns = []; }

    public function clearPlayer(Player $player): void {
        $key = strtolower($player->getName());
        foreach ($this->cooldowns as $eid => &$players) {
            unset($players[$key]);
            if ($players === []) unset($this->cooldowns[$eid]);
        }
        unset($players);
    }
    ...
    public function purgeExpired(): void {
        ...
        foreach ($this->cooldowns as $eid => &$players) {
            foreach ($players as $name => $expire) {
                if ($now >= $expire) unset($players[$name]);
            }
            if ($players === []) unset($this->cooldowns[$eid]);
        }
        unset($players);
